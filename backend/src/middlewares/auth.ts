...
export class Auth {
  ...
  static async PlatformAuth(req: any, res: any, next: () => void) {
    // extract the name and password from basic authorization
    let credentails = auth(req);
    ...
    if (
      credentails &&
      credentails.name === <string>process.env.PLATFORM_AUTH &&
      credentails.pass === <string>process.env.PLATFORM_PASSWORD
    ) {
      next();
    } else {
      sendError(res, ERROR.PLATFORM_UNAUTHORISED);
    }
  }
